 ...
 * <br>
 * URL： api/data/add <br>
 * 参数：params={"datas": [data1, data2...]} <br>
 * 返回：{"state": "success"} 或 {"state": "failed"} <br>
 * 说明：添加新的数据，参数为data数组，成功返回success，失败返回failed，失败不会添加任何data
 * 
...
public class AddAction extends AbstractApiAction {
	...
	public String execute() {
		...
		if (!checkParamsJsonFormat()) {
			System.out.println("[API][api/data/add]: 非法参数(" + params + ")");
